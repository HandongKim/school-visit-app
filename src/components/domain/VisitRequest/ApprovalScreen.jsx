  ...
  return (
    <li key={item.id} className="bg-white shadow rounded-xl p-4 text-sm">
      ...
      <div className="text-gray-700 mb-2">
        시간: {item.breakVisit && item.departureTime
          ? new Date(item.departureTime.seconds * 1000).toLocaleTimeString('ko-KR', { hour: '2-digit', minute: '2-digit' })
          : item.time}
        <br />
        사유: {item.reason} <br />
        유형: {item.type}
      ...
      {mode === 'status' && (
        <div className="text-sm text-blue-600 font-semibold">
          {isApproved
            ? '승인 완료'
            : `승인 대기 중 (${status.homeroom ?? '대기'} / ${status.subject ?? '대기'})`}
        </div>
      )}
    </li>
  );
...
const renderBreakItem = item => (
  ...
  return (
    <div className="max-w-xl mx-auto p-4">
      ...
      {filtered.length === 0 ? (
        <p className="text-center text-gray-500 text-sm">표시할 요청이 없습니다.</p>
      ) : (
      ...
      {mode === 'status' && (() => {
        ...
        return (
          <div className="mt-8">
            <h3 className="text-lg font-semibold mb-4 text-center">쉬는 시간 방문 현황</h3>
            <ul className="space-y-4">
