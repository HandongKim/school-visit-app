...
export default function VisitRequestForm({ userInfo }) {
  ...
  const [form, setForm] = useState({
    ...
    reason: '',
    breakVisit: false,
    departureTime: '',  // datetime-local
  });

  // 학생 리스트 (학년·반 변경 시 로드)
  const [studentList, setStudentList] = useState([]);

  // 학/반 변경된 경우에만 (담임 제외) 학생 불러오기
  useEffect(() => {
    ...
    if (!isHomeroom) loadStudents();
  }, [form.grade, form.class, isHomeroom]);
